...
/*
TelemetryApiService Get the Result of an Active Deployment Telemetry Request
...
func (a *TelemetryApiService) ActiveDeploymentTelemetryGet(ctx context.Context, retrievalKey string) (ActiveDeploymentTelemetryGetResult, *http.Response, error) {
	var (
		...
		localVarReturnValue ActiveDeploymentTelemetryGetResult
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/v1/telemetry/active-deployments/{retrieval_key}"
	localVarPath = strings.Replace(localVarPath, "{"+"retrieval_key"+"}", fmt.Sprintf("%v", retrievalKey), -1)
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v ActiveDeploymentTelemetryGetResult
...
/*
TelemetryApiService Create a New Active Deployment Telemetry Request
...
func (a *TelemetryApiService) ActiveDeploymentTelemetryPost(ctx context.Context, payload ActiveDeploymentTelemetryRequest) (ActiveDeploymentTelemetryResponse, *http.Response, error) {
	var (
		...
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue ActiveDeploymentTelemetryResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/v1/telemetry/active-deployments"
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v ActiveDeploymentTelemetryResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse, newErr
